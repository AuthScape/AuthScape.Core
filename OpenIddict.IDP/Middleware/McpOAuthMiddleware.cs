...
{
    /// <summary>
    /// Middleware to handle MCP OAuth endpoints before OpenIddict takes over.
    /// This intercepts /.well-known/oauth-authorization-server to add registration_endpoint
    /// and handles the dynamic client registration endpoint.
    /// </summary>
    public class McpOAuthMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (path == "/.well-known/oauth-authorization-server/register" && context.Request.Method == "POST")
            {
        ...
        private async Task HandleOAuthMetadata(HttpContext context)
        {
            ...
            var metadata = new
            {
                ...
                registration_endpoint = $"{baseUrl}/.well-known/oauth-authorization-server/register",
        ...
        private async Task HandleClientRegistration(HttpContext context)
        {
            ...
            catch (Exception ex)
            {
                ...
                await WriteError(context, 400, "server_error", "An error occurred during client registration");
            }
        }
