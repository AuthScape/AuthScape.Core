...
public class DynamicsProvider : BaseCrmProvider
{
    ...
    private Dictionary<string, object?> SanitizeFieldsForWrite(string entityName, Dictionary<string, object?> fields)
    {
        ...
        // These are entity primary keys (e.g., accountid on account, contactid on contact) that cannot be
        // sent as direct properties OR as @odata.bind navigation bindings on ANY entity.
        ...
        {
            // Strip direct primary key fields
            ...
            // Strip @odata.bind annotations that reference primary key field names
            // e.g., "accountid@odata.bind" is invalid because accountid is a primary key, not a navigation property
            if (kvp.Key.EndsWith("@odata.bind", StringComparison.OrdinalIgnoreCase))
            {
                var baseFieldName = kvp.Key[..kvp.Key.IndexOf('@')];
                if (readOnlyPrimaryKeys.Contains(baseFieldName))
                {
                    _logger.LogWarning("Stripping invalid @odata.bind for primary key '{FieldName}' from {EntityName} write payload. " +
                        "Primary keys cannot be used as navigation property bindings. " +
                        "Check relationship mapping configuration - the CRM lookup field should be a navigation property (e.g., parentcustomerid_account), not a primary key.",
                        kvp.Key, entityName);
                    continue;
                }
            }
