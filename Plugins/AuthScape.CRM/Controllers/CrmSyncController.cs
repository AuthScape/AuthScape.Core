...
public class CrmSyncController : ControllerBase
{
    ...
    /// </summary>
    [HttpPost]
    public async Task<ActionResult<SyncResultDto>> SyncCrmUserLocations(long connectionId)
    {
        _logger.LogInformation("Starting user location sync from CRM for connection {ConnectionId}", connectionId);

        var result = await _syncService.SyncUserLocationsFromCrmAsync(connectionId);
