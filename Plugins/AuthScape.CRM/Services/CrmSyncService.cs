...
public class CrmSyncService : ICrmSyncService
{
    ...
    /// </summary>
    public async Task<CrmSyncResult> SyncUserLocationsFromCrmAsync(long connectionId)
    {
        var result = new CrmSyncResult();
        var sw = Stopwatch.StartNew();
        string? syncId = null;
        ...
        try
        {
            ...
            foreach (var userExternalId in userExternalIds)
            {
                try
                {
                    ...
                    // Get bcbi_companyid value - try multiple field name formats
                    // Dynamics uses _fieldname_value format for lookup values
                    string? accountGuid = null;
                    var possibleFieldNames = new[]
                    {
                        "_bcbi_companyid_value",  // Dynamics lookup value format
                        "bcbi_companyid",         // Direct field name
                        "_parentcustomerid_value", // Standard parent customer lookup
                        "parentcustomerid"        // Direct parent customer field
                    };
                    ...
                    if (string.IsNullOrEmpty(accountGuid))
                    {
                        ...
                        {
                            _logger.LogWarning("Contact {ContactId} has no bcbi_companyid or parentcustomerid set. Available fields: {Fields}",
                                userExternalId.CrmEntityId,
                                string.Join(", ", contactRecord.Fields.Where(f => f.Key.Contains("company") || f.Key.Contains("parent") || f.Key.Contains("account") || f.Key.Contains("bcbi")).Select(f => $"{f.Key}={f.Value}")));
                        }
                        skippedCount++;
                        processedCount++;
                        result.Stats.TotalProcessed++;
                    ...
                    {
                        // Log this more prominently - this is likely the issue
                        if (processedCount < 10)
                        {
                            _logger.LogWarning("No Location/Company found for account GUID {AccountGuid}. " +
                                "Make sure Locations/Companies are synced to accounts first. " +
                                "Looking for CrmExternalId with CrmEntityId={AccountGuid}, CrmEntityName=account, ConnectionId={ConnectionId}",
                                accountGuid, accountGuid, connectionId);

                            // Check what external IDs we DO have for accounts
                            var existingAccountMappings = await _context.CrmExternalIds
                                .Where(e => e.CrmConnectionId == connectionId && e.CrmEntityName == "account")
                                .Take(5)
                                .ToListAsync();

                            if (existingAccountMappings.Any())
                            {
                                _logger.LogInformation("Sample existing account mappings: {Mappings}",
                                    string.Join(", ", existingAccountMappings.Select(e => $"[{e.AuthScapeEntityType}:{e.AuthScapeEntityId}->account:{e.CrmEntityId}]")));
                            }
                            else
                            {
                                _logger.LogWarning("No CrmExternalIds found for accounts at all! Locations/Companies need to be synced to CRM accounts first.");
                            }
                        }
                        skippedCount++;
                        processedCount++;
                    ...
                    if (user != null)
                    {
                        if (user.LocationId != locationExternalId.AuthScapeEntityId)
                        {
            ...
            result.Success = result.Stats.FailedCount == 0;
            result.Message = $"Sync completed: {updatedCount} users updated, {skippedCount} skipped, {result.Stats.FailedCount} failed";

            _logger.LogInformation("User location sync completed: {Updated} updated, {Skipped} skipped, {Failed} failed",
                updatedCount, skippedCount, result.Stats.FailedCount);
